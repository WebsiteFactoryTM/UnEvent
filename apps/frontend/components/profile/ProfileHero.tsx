    ...
    <div className="glass-card p-6 md:p-8 space-y-6 animate-fade-in-up">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <Avatar className="h-24 w-24 md:h-32 md:w-32 ring-4 ring-primary/20">
          <AvatarImage src={user.avatarURL || undefined} alt={profile.displayName || profile.name} />
        ...
        <div className="flex-1 space-y-3">
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="text-2xl md:text-3xl font-bold">{profile.displayName || profile.name}</h1>
              {isVerified && (
            ...
            {/* Rating */}
            {profile.rating && profile.rating.count && profile.rating.count > 0 && (
              <div className="flex items-center gap-2 text-sm">
                <div className="flex items-center gap-1">
                  <FaStar className="h-4 w-4 text-yellow-500" />
                  <span className="font-semibold">{profile.rating.average?.toFixed(1)}</span>
                </div>
                <span className="text-muted-foreground">
                  · {profile.rating.count} {profile.rating.count === 1 ? "evaluare" : "evaluări"}
                </span>
              </div>
            )}
