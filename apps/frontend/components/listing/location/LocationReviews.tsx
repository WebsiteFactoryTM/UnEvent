      ...
      <div className="flex items-center justify-between">
        ...
        {location.rating && location.reviewCount && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-muted-foreground">({location.reviewCount} recenzii)</span>
          </div>
        )}
      </div>
      ...
      {showReviewForm && (
        <div className="p-4 rounded-lg border border-border space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-1">
              {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                  <FaStar className={`h-6 w-6 ${star <= rating ? "text-yellow-500" : "text-muted"}`} />
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Textarea placeholder="Scrie recenzia ta aici..." className="min-h-[100px]" />
          </div>
      ...
      {approvedReviews.length > 0 ? (
        <div className="space-y-4">
          {approvedReviews.map((review) => {
            ...
            return (
              <div key={review.id} className="p-4 rounded-lg border border-border space-y-3">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 space-y-1">
                    <div className="flex items-center justify-between">
                      <p className="font-semibold">{user?.displayName || user?.name || "Utilizator"}</p>
