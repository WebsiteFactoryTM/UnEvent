    ...
    <div className="glass-card p-4 sm:p-6 space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-balance">{event.title}</h1>
          <div className="flex gap-2 flex-shrink-0">
            {event.status === "approved" && <Badge className="bg-green-500/90 backdrop-blur-sm">Verificat</Badge>}
            {event.tier === "recommended" && <Badge className="bg-blue-500/90 backdrop-blur-sm">Recomandat</Badge>}
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm">
          {event.rating && event.reviewCount && (
            <div className="flex items-center gap-1">
              ...
              <span className="text-muted-foreground">· {event.reviewCount} recenzii</span>
            </div>
          )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            {event.endDate && <p className="text-sm text-muted-foreground">până la {formatDate(event.endDate)}</p>}
          </div>
        </div>
        ...
        {event.capacity && (
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">{event.capacity.total} locuri</p>
            </div>
          </div>
        )}
