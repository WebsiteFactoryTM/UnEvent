...
import type { Event, Location } from "@/types/payload-types";
...
export default function EventLocationMap({ event }: EventLocationMapProps) {
  const cityName =
    typeof event.city === "object" && event.city ? event.city.name : "";
  const venue =
    typeof event.venue === "object" ? (event.venue as Location) : null;
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="font-medium">
              {event.address || "Adresă nedisponibilă"}
            </p>
