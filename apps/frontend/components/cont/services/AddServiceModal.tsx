...
import { serviceFormSchema, defaultServiceFormValues, type ServiceFormData } from "@/forms/service/schema"
...
export function AddServiceModal({ open, onOpenChange }: AddServiceModalProps) {
  ...
  const handleSaveDraft = () => {
    ...
    // await saveServiceDraft(currentData)
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="w-[95vw] md:w-[92vw] lg:w-[90vw] max-w-[calc(100%-2rem)] sm:max-w-[95vw] md:max-w-[92vw] lg:max-w-[90vw] xl:max-w-[1600px] h-[90vh] max-h-[90vh] p-0 flex flex-col">
        <DialogHeader className="px-4 sm:px-6 lg:px-8 pt-4 sm:pt-6 pb-3 sm:pb-4 shrink-0">
          <DialogTitle className="text-xl sm:text-2xl">Adaugă serviciu</DialogTitle>
          ...
          <FormProvider {...methods}>
            <form onSubmit={handleSubmit(onSubmit, handleError)} className="flex flex-col flex-1 min-h-0">
              <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
                ...
                <div className="flex items-center justify-between gap-2 sm:gap-3 px-4 sm:px-6 lg:px-8 py-3 sm:py-4 border-t bg-muted/30 shrink-0">
                  <Button
                    type="button"
                    variant="outline"
