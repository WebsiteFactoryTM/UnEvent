...
export function SubmitDone({ onClose }: SubmitDoneProps) {
  ...
  const handleClose = () => {
    toast({
      title: "Listare serviciu trimisă",
      description: "Vei fi notificat când listarea va fi aprobată.",
    })
    onClose()
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[400px] px-6 py-12 text-center">
      ...
      <p className="text-sm text-muted-foreground max-w-md mb-8">
        De obicei, procesul de verificare durează 24-48 de ore. Te vom notifica imediat ce serviciul tău va fi disponibil pe platformă.
      </p>
      ...
      <div className="mt-8 p-4 border rounded-lg bg-muted/20 max-w-md">
        <p className="text-xs text-muted-foreground">
          Între timp, poți continua să îți completezi profilul sau să adaugi alte servicii.
        </p>
      </div>
    </div>
  )
