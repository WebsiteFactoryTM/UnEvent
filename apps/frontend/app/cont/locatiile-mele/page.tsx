  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-foreground">Locațiile mele</h1>
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-left py-3 px-4 text-sm font-semibold text-foreground">Locație (oraș)</th>
            ...
            <tbody>
              {locations.map((location) => (
                <tr key={location.id} className="border-b border-border/50 hover:bg-muted/50 transition-colors">
                  <td className="py-4 px-4">
                    <div className="font-medium text-foreground">{location.title}</div>
                  </td>
                  <td className="py-4 px-4">
                    <div className="text-sm text-muted-foreground max-w-xs truncate">{location.description}</div>
                  </td>
                  <td className="py-4 px-4">
                    <div className="text-sm text-foreground">{getCityName(location.city)}</div>
                  </td>
                  <td className="py-4 px-4">
                    <div className="text-sm text-muted-foreground">{formatDate(location.createdAt)}</div>
                  </td>
                  <td className="py-4 px-4">
                    <div className="text-sm text-foreground font-medium">{location.views?.toLocaleString() || 0}</div>
                  </td>
                  <td className="py-4 px-4">
                    <span
                  ...
                  <td className="py-4 px-4">
                    <div className="flex items-center justify-end gap-2">
                      <Button variant="ghost" size="sm" onClick={() => handleView(location)} className="h-8 w-8 p-0">
                        <FaEye className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="sm" onClick={() => handleEdit(location)} className="h-8 w-8 p-0">
          ...
          {locations.length === 0 && (
            <div className="text-center py-12">
              <p className="text-muted-foreground">Nu ai încă locații adăugate.</p>
          ...
          {locations.map((location) => (
            ...
            >
              ...
              <div className="flex items-start justify-between gap-3">
                <h3 className="font-semibold text-foreground text-lg flex-1">{location.title}</h3>
                <span
              ...
              <div className="grid grid-cols-2 gap-3 pt-2 border-t border-border/30">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-foreground">{getCityName(location.city)}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  <FaChartLine className="h-4 w-4 text-muted-foreground" />
                  <span className="text-foreground font-medium">{location.views?.toLocaleString() || 0}</span>
                </div>
      ...
      {locations.some((l) => l.status === "rejected") && (
        <SectionCard title="Locații respinse" description="Locații care necesită atenție">
          <div className="space-y-4">
            {locations
              .filter((l) => l.status === "rejected")
              .map((location) => (
                <div key={location.id} className="p-4 rounded-lg bg-red-500/10 border border-red-500/30">
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground mt-1">
                        ...
                        {location.rejectionReason || "Nu a fost specificat un motiv."}
                      </p>
                    </div>
                    <Button
                      variant="outline"
