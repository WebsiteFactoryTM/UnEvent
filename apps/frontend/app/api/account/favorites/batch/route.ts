...
export async function GET(req: NextRequest) {
  ...
  try {
    // Forward targetKeys query parameter
    const url = new URL(`${payloadUrl}/api/favorites/checkBatch`);
    const searchParams = new URL(req.url).searchParams;
    const targetKeys = searchParams.get("targetKeys");

    if (!targetKeys) {
      return new Response(
        JSON.stringify({ error: "targetKeys parameter is required" }),
        {
          status: 400,
          headers: {
    ...
    url.searchParams.set("targetKeys", targetKeys);
    ...
    if (!res.ok) {
      const errorMessage =
        ...
        responseData.error ||
        "Failed to fetch batch favorites";
