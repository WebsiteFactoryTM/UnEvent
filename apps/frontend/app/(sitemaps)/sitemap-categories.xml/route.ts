...
async function fetchCitiesAndTypes() {
  try {
    ...
    const cities = citiesResponse.ok ? (await citiesResponse.json()).docs || [] : [];
    const types = typesResponse.ok ? (await typesResponse.json()).docs || [] : [];
...
export async function GET() {
  ...
  for (const city of topCities) {
    for (const type of locationTypes.slice(0, 10)) { // Top 10 location types
      urls.push({
  ...
  for (const city of topCities) {
    for (const type of serviceTypes.slice(0, 10)) { // Top 10 service types
      urls.push({
