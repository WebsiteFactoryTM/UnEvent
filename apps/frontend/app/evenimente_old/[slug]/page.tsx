...
export async function generateMetadata({
  ...
  const cityName =
    typeof event.city === "object" && event.city ? event.city.name : "România";
  const description = event.description
    ? event.description.slice(0, 160)
    : `${event.title} - Eveniment în ${cityName}`;
      ...
      <div className="min-h-screen bg-background">
        <div className="container mx-auto px-4 py-6 space-y-8">
          ...
          {event.youtubeLinks && event.youtubeLinks.length > 0 && (
            <EventVideos youtubeLinks={event.youtubeLinks} />
          )}
          ...
          <EventRecommendations
            cityName={cityName}
            similarEvents={mockSimilarEvents}
          />
        </div>
      </div>
    </>
