...
import { geocodeAddress, debounce, GeocodingResult, GeocodingError } from "@/lib/geocoding";
...
export const AddressInput: React.FC<AddressInputProps> = ({
      ...
      try {
        ...
        if ("error" in result) {
          ...
          if (onLocationInfoChange && result.address_components) {
            const { city, county } = extractLocationInfo(result.address_components);
  ...
  const extractLocationInfo = (addressComponents: any[]): { city: string; county: string } => {
    let city = "";
    let county = "";
    ...
    <div className={cn("space-y-2", className)}>
      ...
      <div className="relative">
        <Input
          ...
          disabled={disabled}
          className={cn("pr-20", error && "border-destructive", hasGeocoded && "border-green-500")}
        />
        ...
        <div className="absolute right-2 top-1/2 transform -translate-y-1/2 flex items-center gap-1">
          ...
          {hasGeocoded && !isGeocoding && <Check className="h-4 w-4 text-green-500" />}

          {geocodingError && !isGeocoding && <AlertCircle className="h-4 w-4 text-destructive" />}
      ...
      <p className="text-xs text-muted-foreground">
        Introduceți adresa completă pentru a obține automat coordonatele. Coordonatele vor fi
        calculate automat când adresa este găsită.
      </p>
    </div>
  );
};
